...
public class PruebasSwing extends JPanel implements ActionListener, KeyListener{
	...
	protected void paintComponent(Graphics g){
		...
		// Se pintan las paradas y las lineas que los unen, empezando por la parada con id = 0
	...
	private void pintarPuntos(int width, int height,HashSet<Parada> set,Parada parada) {
		...
		// Se añade la parada al set de paradas visitadas y se determinan las coordenadas en las que se va a pintar
		...
		// Se dan las propiedades al boton, es posible que haya cosas de mas en esta parte
		...
		// Se pintan los puntos de las paradas, actualmente no visibles porque los tapan los botones

		g1.setPaint(parada.color);
		g1.fill(new Ellipse2D.Double(X+2.5,Y+2.5,10,10));
		
		// Para cada conexion de cada parada se pintan las lineas desde esa parada hasta el destino de la conexion
		...
		for (Conexion conexion : parada.conexiones) {
			...
			if(!parada.color.equals(paradaAux.color)) {
				...
				Line2D linea = new Line2D.Float(Xaux.intValue()+5, Yaux.intValue()+5, X.intValue()+5, Y.intValue()+5);
				g1.draw(linea);
				g1.setStroke(new BasicStroke(3f));
	...
	public void actionPerformed(ActionEvent e) {
		...
		// moveUp mueve la "camara" hacia arriba, en realidad lo que hace es disminuir la y del mapa
		...
		// moveUp mueve la "camara" hacia abajo, en realidad lo que hace es aumentar la y del mapa
		...
		// moveRight mueve la "camara" hacia derecha, en realidad lo que hace es disminuir la x del mapa
		...
		// moveLeft mueve la "camara" hacia izquierda, en realidad lo que hace es aumentar la x del mapa
